...
public class DeploymentOpenShiftConverter implements KubernetesToOpenShiftConverter {
                ...
                if (containerToImageMap.size() != 0) {
                    if (mode.equals(PlatformMode.openshift)) {
                        for (Map.Entry<String, String> entry : containerToImageMap.entrySet()) {
                            ...
                            // Only add Image triggers to main containers
                            if(isApplicationContainer(image.getFullName())) {
    ...
    /**
     * Helper method to check whether it's application container or sidecar.
     *
     * Just handling this case when ImageName is taken from project
     * artifactId, since this conversion of Deployment -> DeploymentConfig
     * conversion is going to get refactored anyway.
     *
     * @param imageName
     * @return boolean value indicating whether container is main or not.
     */
    private boolean isApplicationContainer(String imageName) {
        return imageName.contains(project.getArtifactId());
    }
}
