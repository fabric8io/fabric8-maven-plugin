...
However, there is one configuration option which is shared by all generators: The base image from where to start from when creating the images. By default the generators make an opinionated decision for the base image and which are described in the respective generator section.
