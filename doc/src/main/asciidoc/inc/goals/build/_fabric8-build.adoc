...
| `Type` | Description
 ...
 | `RegistryImage`
| RegistryImage is built and pushed to the registry if the registry is correctly authenticated. Default registry used is https://hub.docker.com/[Dockerhub]. More about https://static.javadoc.io/com.google.cloud.tools/jib-core/0.10.1/com/google/cloud/tools/jib/api/RegistryImage.html[RegistryImage].

| `TarImage`
| TarImage archive is built if the registry is not authenticated correctly. The tar archive is built and can be found in `outputDirectory`, the default value is `target/docker`. More about https://static.javadoc.io/com.google.cloud.tools/jib-core/0.10.1/com/google/cloud/tools/jib/api/TarImage.html[TarImage].
|===
...
user needs to specify release version 11 in their `maven-compiler-plugin` configuration in their project pom.
