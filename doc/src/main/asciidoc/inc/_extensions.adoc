...
* *Generators* are used to auto create or customize image configuration when creating Docker images. They are a bit like http://docs.spring.io/spring-boot/docs/current/reference/htmlsingle/#using-boot-generator-poms[Spring Boot Generator POMs] as they can be enabled or disabled by declaring a Maven dependency. Generators are able to examine the build and to _detect_ certain feature like whether Spring boot application is build or a plain war file. Depending on the collected informations a base image or the exposed ports are selected automatically for creating a image build configuration.
