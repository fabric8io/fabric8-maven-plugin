...
For more advanced requirement's an _XML configuration_ provides additional configuration options which can be added to the `pom.xml`.
For the full power in order to tune all facets of the creation external _resource fragments_ and _Dockerfiles_ can be used. The docker compose configuration can be used to bring up the docker compose deployments on Kubernetes/OpenShift cluster.
...
* *Docker Compose* configuration can bring up the docker compose deployments on Kubernetes/OpenShift cluster. This requires minimum to no knowledge of Kubernetes/OpenShift deployment process.
...
| *Docker Compose*
| All above methods can works with docker compose configuration. As `dokcercompose.yml` can have `:build` key that specifies how to build docker image but at present this feature is not supported.
| Plugin converts docker compose descriptors of application definitions into more detailed Kubernetes/OpenShift deployment resources. These resource descriptors or externally YAML files can be used to specify application skeleton. This skeleton is then filled by Enrichers which add labels and more. Maven properties within these files are resolved to thier values. With this model you can use every Kubernetes / OpenShift resource object with all their flexibility, but still get the benefit of adding build informations.
|===
...
The fourth configuration option is to provide an external Docker Compose file. Following are either ways to specify docker-compose files.

1. Keep Docker Compose file into `src/main/fabric8-compose` directory in project space.

2. Locate Docker Compose file path with in the project directory as shown in following `pom.xml` example.
...
As you can see in `pom.xml`, `composeFile` section under plugin `configuration` is used to provide the Docker Compose file path. It should be relative path from `pom.xml`.

Next step is `execution` goals configuration. It has to has `resource` goal so that plugin can process Docker Compose file and generate Kubernetes, OpenShift resource descriptors using <<fabric8:resource>> goal.
