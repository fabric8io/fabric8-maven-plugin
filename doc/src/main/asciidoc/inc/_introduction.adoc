...
* *Docker compose* are user provided docker compose YAML files. This feature can bring up the docker compose deployments on Kubernetes &amp; Openshift cluster with minimum or no knowledge of Kubernetes/Openshift deployment process.
...
2. Locate docker compose file with in project directory as shown in following `pom.xml` example.
...
As you can see, `composeFile` section under plugin `configuration` has used to provide docker compose file location and location should be relative path from `pom.xml`.

Next step is `execution` goals config which requires `resource` goal at least, so that plugin can process docker compose file and generate Kubernetes, Openshift resource descriptors using <<fabric8:resource>> goal.
Thanks to http://kompose.io[kompose] tool for building this feature.
