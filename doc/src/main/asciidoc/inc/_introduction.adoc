...
1. Keep docker compose file into `src/main/fabric8-kompose` directory with in project directory.
...
As you can see, there is `composeFile` section in plugin configuration in `pom.xml` which locates docker-compsoe file is and location should be relative path from `pom.xml`.

Next is `execution` goals config which requires `resource` goal at least, so that docker-compose file can get process.
