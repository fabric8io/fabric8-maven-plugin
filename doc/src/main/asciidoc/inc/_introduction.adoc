...
* *Docker compose* are user provided docker compose YAML files. This feature can bring up the docker compose based deployments on Kubernetes &amp; Openshift cluster with minimum or no knowledge of Kubernetes/Openshift deployment process.
...
2. Locate docker compose file with in project directory with following `pom.xml` example.
...
As you can see, there is `composeFile` section under plugin `configuration` in `pom.xml` to provie docker-compsoe file location and location should be relative path from `pom.xml`.

Next step is `execution` goals config which requires `resource` goal at least, so that docker-compose file can get process.
