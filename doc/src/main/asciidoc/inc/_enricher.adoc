...
* The following annotations:
  - `discovery.3scale.net/discovery-version`: the version of the 3scale discovery process. By default its "v1"
  - `discovery.3scale.net/scheme`: the scheme of the service, this can be "http" or "https". "http" by default.
  ...
  - `discovery.3scale.net/description-path`: the path to the service description document (OpenAPI/Swagger). The path can either be relative or an external full URL. By default the description is picked up from the root context (/)
...
| *discoverable*
| Sets the discovery.3scale.net label to either "true" or "false". If set to "true" 3scale will try to discover this service. If not given then the enricher tries to autodetect the whether it can make the service discoverable. Currently project using camel-rest can be auto exposed this way. If not set and nothing can be autodetected this enricher is a no-op.
| 
