...
This enricher adds additional SCM related metadata to deployments and services.  These metadata will be added only if
`<scm>` information is present in the maven `pom.xml` of the project.

The following annotations will be added to the kubernetes deployment and service

.Maven SCM Enrichers Annotation Mapping
[cols="2,2"]
|===
| Maven SCM Info | Annotation
...
This enricher adds additional Issue Management related metadata to deployments and services.
These metadata will be added only if the `<issueManagement>` information in available in maven `pom.xml` of the project.
...
[cols="2,2"]
|===
| Maven Issue Tracker Info | Annotation
